...
def find_urls(any_str):
	list_of_urls=[]
	match= re.findall(r'(http[s]?:\/\/(?:[a-zA-Z]|[0-9]|[$-_@.&+]|[!*\(\),]|(?:%[0-9a-fA-F][0-9a-fA-F]))+)', any_str)
	if match:
		for url in match:
...
def get_umsi_data():
	try:
		with open(cache_filename, 'r') as outfile: 
			cache2= json.load(outfile)
			return cache2['umsi_directory_data']
	except:
		dictionary_lst=[]
		for each_page in range(12):
			response = requests.get("https://www.si.umich.edu/directory?field_person_firstname_value=&field_person_lastname_value=&rid=All&page="+str(each_page), headers={'User-Agent': 'SI_CLASS'})
			htmldoc = response.text
			dictionary_lst.append(htmldoc)
	try:
		cache2= json.load(cache_filename)
		cache2['umsi_directory_data'] = dictionary_lst
		with open(cache_filename, 'w') as outfile:
			json.dump(cache2, outfile)
	except:
		cache_diction={}
		cache_diction['umsi_directory_data']= dictionary_lst
		with open(cache_filename, 'w') as outfile:
			json.dump(cache_diction, outfile)
...
def get_five_tweets(any_str1):
	try:
		with open(cache_filename, 'r') as outfile: 
			cache2= json.load(outfile)
			try:
				if cache2["most_recent"] == any_str1:
					five_tweets= [tweet["text"] for tweet in cache2["twitter_University of Michigan"]["statuses"]][:5]
					return five_tweets
				else:
					with open(cache_filename, 'r') as outfile:
						cache_dict = json.load(outfile)
						public_tweets = api.search(q=any_str1)
						cache_dict["most_recent"] = any_str1
						cache_dict["twitter_University of Michigan"] = public_tweets
						with open(cache_filename, 'w') as outfile:
							json.dump(cache_dict, outfile)
						tweet_list = public_tweets["statuses"]
						five_tweets= [tweet["text"] for tweet in tweet_list][:5]
						return five_tweets
		...
		public_tweets = api.search(q=any_str1)
		cache_diction={}
		cache_diction['twitter_University of Michigan']= public_tweets
		cache_diction['most_recent'] = any_str1
		with open(cache_filename, 'w') as outfile:
			json.dump(cache_diction, outfile)
		tweet_list = public_tweets["statuses"]
		five_tweets= [tweet["text"] for tweet in tweet_list][:5]
...
for each in five_tweets:
	m= find_urls(each)
	tweet_urls_found.extend(m)
